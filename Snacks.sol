...
contract Snacks is ERC20, Ownable {
    ...
    uint256 private _mutiplier = 1000000000000000000 * 0.000001;
    ...
    function calculateBuyAmount(uint256 amount) view public returns (uint256) {
        uint256 start = totalSupply().div(10 ** decimals()) + 1;
        uint256 end = totalSupply().div(10 ** decimals()) + amount;
        return amount.mul(start + end).div(2);
    }

    function calculateRedeemAmount(uint256 amount) view public returns (uint256) {
        uint256 start = totalSupply().div(10 ** decimals()) ;
        uint256 end = totalSupply().div(10 ** decimals()) - amount + 1;
        return amount.mul(start + end).div(2);
    }
    ...
    function buy(uint256 amount) public {
        // calculate zoinks amount to get amount of snacks
        uint256 zoinksAmount = calculateBuyAmount(amount).mul(_mutiplier);
        ...
        uint256 snackAmount = amount.mul(10**decimals());
    ...
    function redeem(uint256 amount) public {
        // calculate zoinks amount to get amount of snacks
        uint256 zoinksAmount = calculateRedeemAmount(amount).mul(_mutiplier);
