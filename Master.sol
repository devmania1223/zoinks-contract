...
contract CakeToken is BEP20('ZoinksSwap Token', 'Cake') {
...
contract MasterChef is Ownable {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if (user.lastWithdrawDate == 0 || (user.lastWithdrawDate + 1 days > block.timestamp)) {
                pool.lpToken.safeTransfer(seniorageAddress, _amount.div(2));
                pool.lpToken.safeTransfer(address(msg.sender), _amount.div(2));
            } else {
                pool.lpToken.safeTransfer(seniorageAddress, _amount.div(10));
                pool.lpToken.safeTransfer(address(msg.sender), _amount.mul(9).div(10));
            }
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        if (user.lastWithdrawDate == 0 || (user.lastWithdrawDate + 1 days > block.timestamp)) {
            ...
            pool.lpToken.safeTransfer(address(msg.sender), user.amount.div(2));
        } else {
            pool.lpToken.safeTransfer(seniorageAddress, user.amount.div(10));
            pool.lpToken.safeTransfer(address(msg.sender), user.amount.mul(9).div(10));
        }
        user.lastWithdrawDate = block.timestamp;
